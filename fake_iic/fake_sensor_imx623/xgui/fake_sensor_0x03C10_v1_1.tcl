...
proc init_gui { IPINST } {
  ...
  set REG_DEVICE_ADDR [ipgui::add_param $IPINST -name "REG_DEVICE_ADDR" -parent ${Page_0}]
  set_property tooltip {The address of the register that stores the device address} ${REG_DEVICE_ADDR}
  set DEVICE_ADDR_BIT [ipgui::add_param $IPINST -name "DEVICE_ADDR_BIT" -parent ${Page_0}]
  set_property tooltip {Device Addr Bit(Whether the device address is stored in the upper 7 bits or the lower 7 bits)} ${DEVICE_ADDR_BIT}
  ipgui::add_param $IPINST -name "SDA_T_POLARITY" -parent ${Page_0}
...
proc update_PARAM_VALUE.DEVICE_ADDR_BIT { PARAM_VALUE.DEVICE_ADDR_BIT } {
	# Procedure called to update DEVICE_ADDR_BIT when any of the dependent parameters in the arguments change
}

proc validate_PARAM_VALUE.DEVICE_ADDR_BIT { PARAM_VALUE.DEVICE_ADDR_BIT } {
	# Procedure called to validate DEVICE_ADDR_BIT
	return true
}
...
proc update_MODELPARAM_VALUE.DEVICE_ADDR_BIT { MODELPARAM_VALUE.DEVICE_ADDR_BIT PARAM_VALUE.DEVICE_ADDR_BIT } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.DEVICE_ADDR_BIT}] ${MODELPARAM_VALUE.DEVICE_ADDR_BIT}
}
